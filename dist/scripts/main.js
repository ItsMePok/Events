...
// scripts/time.ts
import { GameMode as GameMode2, world } from "@minecraft/server";
...
function PokeErrorScreen(player, error, backTo) {
  ...
  if (!error) {
    error = { translate: `translation.poke_events:errorGeneric` };
  }
  UI.title({ translate: `translation.poke_events:errorGeneric` });
  UI.body(error);
  UI.button({ translate: `translation.poke_events:goBack` }, `textures/poke/common/left_arrow`);
  UI.button({ translate: `translation.poke_events:bossEventClose` }, `textures/poke/common/close`);
...
var PokeCustomEventId = `poke_events:customEvents`;
function PokeTimeCheck(event, player, claimCheck) {
  if (event == null || event.dates == void 0) {
    PokeErrorScreen(player, void 0, world.setDynamicProperty(PokeCustomEventId, JSON.stringify([])));
    return;
  }
  ...
  for (let i = event.dates.length; i > -1; i--) {
    ...
    if (claimCheck) {
      if (!event.gift)
        continue;
      if (player.hasTag(`poke_events:${currentTime.getFullYear()}E-${event.id}`))
        continue;
    }
    return true;
  }
...
function PokeTimeDebug(player) {
  ...
  UI.show(player).then((response) => {
    ...
    if (response.selection == selection) {
      ...
      selection++;
    if (response.selection == selection) {
      player.setDynamicProperty(`poke_events:birthday`, JSON.stringify({ day: 1, month: 0, id: player.id, announce: false, name: player.name, style: "normal", year: void 0 }));
      return;
    } else
      selection++;
    if (response.selection == selection) {
...
function PokeTimeConfigUIMainMenu(player) {
  let currentTime = new Date(Date.now() + PokeTimeZoneOffset(player));
  let UI = new ActionFormData2().body({ translate: `translation.poke_events:timeUiMainMenuBody`, with: { rawtext: [PokeTimeGreeting(currentTime, player), { text: player.name }, { text: `${currentTime.toDateString()}, ${currentTime.toLocaleTimeString()}` }] } });
...
function PokeTimeGreeting(date, player, event, generic) {
  ...
  let morningGreeting = { translate: `translation.poke_events:timeMorningGreet` };
  let noonGreeting = { translate: `translation.poke_events:timeNoonGreet` };
  let nightGreeting = { translate: `translation.poke_events:timeNightGreet` };
  switch (hour) {
...
function PokeTimeEventInfoMenu(event, player) {
  let timeLeft = ``;
  let UI = new ActionFormData2();
  let giftString = { translate: `translation.poke_events:timeEventGift` };
  if (!event.gift) {
    giftString = { text: `` };
  }
  UI.body(
...
function PokeTimeGetAllEvents() {
  let PokeDefaultHolidays = [];
  let EventPointerId = `poke_events:pointer`;
  if (typeof world.getDynamicProperty(EventPointerId) == "string") {
    let EventPointers = JSON.parse(world.getDynamicProperty(EventPointerId).toString());
    for (let i = EventPointers.length - 1; i > -1; i--) {
      let customEvents = JSON.parse(world.getDynamicProperty(EventPointers.at(i)).toString());
  ...
  UI.show(player).then((response) => {
    if (response.canceled && event.id == ``) {
      ...
      if (!providedEvent) {
        ...
        if (world.getDynamicProperty(PokeCustomEventId) != void 0 || world.getDynamicProperty(PokeCustomEventId) != `[]`) {
        }
...
// scripts/main.ts
world2.afterEvents.playerJoin.subscribe((data) => {
  let birthdays = JSON.parse(world2.getDynamicProperty(`poke_events:birthdays`).toString());
  system.runTimeout(() => {
      ...
      if (player.id == data.playerId) {
        ...
        birthdays.forEach((birthday) => {
          if (birthday.day == currentTime.getDate() && birthday.month == currentTime.getMonth()) {
            ...
            player.sendMessage({ translate: `translation.poke_events:birthdayAnnounce`, with: { rawtext: [PokeTimeGreeting(currentTime, player, void 0, true), { text: player.name }, name] } });
          }
        });
      }
    });
...
world2.beforeEvents.worldInitialize.subscribe((event) => {
  let birthdayProperty = world2.getDynamicProperty(`poke_events:birthdays`);
  if (typeof birthdayProperty != "string")
    world2.setDynamicProperty(`poke_events:birthdays`, `[]`);
  if (typeof world2.getDynamicProperty(`poke_events:customEvents`) != "string") {
    world2.setDynamicProperty(`poke_events:customEvents`, "[]");
    console.warn(`Custom events were invalid; resetting to default (Ignore if this world was just created) || Poke-Calendar`);
  } else {
    try {
      JSON.parse(world2.getDynamicProperty(`poke_events:customEvents`)?.toString());
    } catch {
